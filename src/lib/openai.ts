...
Given a headline, its source, and (if available) the scraped article text, write a thorough plain-language summary covering: what happened (all key facts and figures, not just the headline claim), who is involved, when and where it happened, why it matters, and any relevant background or context (e.g. what led to this, reactions from key parties, what happens next) that appears in the source material. Use as much of the available detail as is present in the input — do not compress a detailed article down to a couple of generic sentences. Write in flowing prose paragraphs (2-4 paragraphs for a well-sourced article; shorter only if the source material itself is thin), like natural storytelling rather than a dry list of facts, not bullet points or markdown headings. Do not repeat the headline verbatim. If the full article text is unavailable, summarize as best you can from the headline and snippet, and never invent facts not present in the input.`,
